...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function deleteAllRecords() {
        if (!confirm('确定要删除所有短信记录吗？此操作不可恢复！')) {
            return;
        }
        showPasswordModal();
    }
    ...
    function viewSMSDetail(smsId) {
        ...
        fetch(`/v1/sms/${smsId}`)
            ...
            .then(data => {
                ...
                setTimeout(() => showSMSDetailModal(data), 50);
            })
            .catch(error => {
                // 关闭加载中模态框
                closeModal('loadingModal');
    ...
    function showSMSDetailModal(data) {
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <div class="detail-group">
                                        ...
                                        <div class="detail-value">
                                            ${data.extracted_code ?
                `<code style="background: #f8f9fa; padding: 2px 4px; border-radius: 3px; font-family: monospace; cursor: text; user-select: text;">${data.extracted_code}</code>` :
                '<span class="badge bg-secondary">未提取</span>'}
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="modal-footer">
                            ${data.extracted_code ?
                `<button type="button" class="btn btn-success copy-code">
                                            <i class="fas fa-copy me-1"></i> 复制验证码
                                        </button>` : ''}
        ...
        const modalElement = document.getElementById('smsDetailModal');
        const modal = new bootstrap.Modal(modalElement);
        modal.show();

        const copyButton = modalElement.querySelector('.copy-code');
        if (copyButton) {
            copyButton.addEventListener('click', function () {
                const codeElement = modalElement.querySelector('code');
                if (codeElement) {
                    selectText(codeElement);
                    showToast('info', '验证码已选中', '请使用 Ctrl+C (桌面) 或长按复制 (移动设备)');
                }
            });
        }
    ...
    async function copyToClipboard(text) {
        ...
        // Fallback to execCommand with input element
